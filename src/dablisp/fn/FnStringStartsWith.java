...
public class FnStringStartsWith extends Fn {

    @Override
    public void apply(int argc, Stack stack, Env env, Env fenv) {
        checkArgCount(argc, 2);
        Vector pattern = stack.pop().asStr();
        Vector str = stack.pop().asStr();
        if (str.getStrValue().startsWith(pattern.getStrValue())) {
            stack.push(Symbol.T);
        } else {
    ...
    public String getDocstring() {
        return "(" + getName() + " pattern str) - function -  returns true if STR starts with PATTERN";
    }
