...
public class FnNumeq extends Fn {
    ...
    public void apply(int argc, Stack stack, Env env, Env fenv) {
        if (argc == 0) {
            ...
            Number first = stack.pop().asNumber();
            Symbol result = Symbol.T;
            for (int i = 1; i < argc; i++) {
                Number next = stack.pop().asNumber();
                if (result == Symbol.T && !first.eql(next)) {
                    result = Symbol.NIL;
                }
            }
            stack.push(result);
    ...
    public String getDocstring() {
        return "(" + getName() + " &rest args) - function -  returns true if all args are equal";
    }
