...
public class SpecialFormIf extends SpecialForm {
    ...
    public void compile(Obj args, CompiledCode code, VM vm, CompileEnv env) {
        ...
        int labelelse = code.createLabel();
        int labelend = code.createLabel();
        code.emitJumpF(labelelse);
        Compiler.compile(args.second(), code, vm, env);
        code.emitJump(labelend);
        code.emitLabel(labelelse);
        Compiler.compile(args.length() == 2 ? Symbol.NIL : args.third(), code, vm, env);
        code.emitLabel(labelend);
    }
